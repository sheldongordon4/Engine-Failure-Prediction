...
import joblib
pd.set_option("styler.render.max_elements", 500000)  # or any value > your cell count
...
# Extract and save engine IDs
    ...
    df_predict = df.copy()
    df_predict = df_predict.drop(columns=features_to_drop)
    ...
    st.metric("Engines Needing Inspection", int((df_predict["action"] == "Inspect").sum()))
    ...
    df_predict["engine_id"] = df["engine_id"]  # Add it back for reference
    latest_rul = df_predict[df_predict['engine_id'] == selected_engine].sort_values("cycle_time")["predicted_rul"].iloc[-1]
